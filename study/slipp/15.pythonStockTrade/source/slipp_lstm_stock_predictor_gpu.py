...
saver_dir_name = "checkpoints-gpu"
...
def lstm_cell():
    ...
    cell = tf.contrib.cudnn_rnn.CudnnCompatibleLSTMCell(num_units=rnn_cell_hidden_dim)
...
def update_buy_list(predict_date):
    fname = "top10_buy_list_{}.txt".format(predict_date.replace('-', ''))
    with open(fname, "wt") as f:
        for stock in gain_rate_top_10:
            line = "buy;{};market;10;0;prebuy;{};{}\n".format(stock.code, stock.gain_rate, stock.price)
            f.write(line)
    log.info("{} updated".format(fname))
...
predict_date = '2018-11-12'
stock_data = SlippStockCrawler().select_stock_price_model_for_predict(predict_date=predict_date)

log.info("data length : {}".format(len(stock_data)))
for code in stock_data:
    ...
    size = len(df)
    if(size < seq_length):
        log.warn('data is not enough : {}'.format(size))
        continue
    ...
    last_close_price = df['Adj_Close'].values[-1]
...
log.info("append_gain_rate_top_10 : {}".format(gain_rate_top_10))
update_buy_list(predict_date)
