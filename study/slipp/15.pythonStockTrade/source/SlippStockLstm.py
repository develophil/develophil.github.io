...
stock_dict = preprocess_data(SlippStockCrawler().select_stock_price_model())
...
def build_and_train_network(lstm_sizes, vocab_size, embed_size, epochs, batch_size,
                            learning_rate, keep_prob, train_x, valid_x, train_y, valid_y):
        ...
        for e in range(epochs):
            ...
            for ii, (x, y) in enumerate(utl.get_batches(train_x, train_y, batch_size), 1):
                ...
                if (ii + 1) % n_batches == 0:
                    ...
                    for xx, yy in utl.get_batches(valid_x, valid_y, batch_size):
                        feed = {inputs_: xx,
                                labels_: yy[:, None],
                                keep_prob_: 1,
...
def train_val_test_split(stock_df, split_frac, random_seed=None):
    ...
    train_x, valid_x = messages_shuf[:split_idx], messages_shuf[split_idx:]
    train_y, valid_y = labels_shuf[:split_idx], labels_shuf[split_idx:]
...
for i, code in enumerate(stock_codes):
    ...
    stock_df = stock_codes[code]
    train_x, valid_x, test_x, train_y, valid_y, test_y = train_val_test_split(stock_codes, split_frac=0.70)
    ...
    with tf.Graph().as_default():
        build_and_train_network(lstm_sizes, epochs, batch_size, learning_rate, keep_prob, train_x, valid_x, train_y, valid_y)
