...
for code in stock_dict:
    ...
    if( (len(raw_dataframe) * 0.1) - seq_length > 0):
        ...
        price_a = stock_info[:, 0:4]
        price_b = stock_info[:, -4:]

        price_all = np.concatenate((price_a, price_b), axis=1)

        norm_price = min_max_scaling(price_all)  # 가격형태 데이터 정규화 처리
        print("price_all.shape: ", price_all.shape)
        print("price_all[0]: ", price_all[0])
